...
6. Discussion(2 Users,  MIMO Channel, Only Common Messages)
...
\begin{align*}
\frac{\partial	(\displaystyle\sum_{i=1}^{2}	MSE^{(c)}_{k}) }{\partial \textbf{v}^{(c)}_{2}} = 0
\end{align*}

\textbf{v}^{*(c)}_{2} 	= &	\bigg[ 2	\textbf{H}^{H}_{12}	\textbf{g}^{(c)}_{1}	\textbf{g}^{H(c)}_{1}		\textbf{H}_{12}
